...
::mediapipe::Status RunMPPGraph(
    grpc::ServerWriter<qoin::FaceMeshReply>* server_writer) {
  ...
  graph.ObserveOutputStream(kOutputLandmarks, [&](const mediapipe::Packet& p) {
    ...
    for (int i = 0; i < landmarks.size(); i++) {
      ...
      //reply.mutable_normalized_landmark_list()->CopyFrom(landmarks[i]);
...
class FaceMeshServiceImpl final : public qoin::FaceMesh::Service {
  grpc::Status FaceMeshStream(grpc::ServerContext* context,
                              const qoin::FaceMeshRequest* request,
                              grpc::ServerWriter<qoin::FaceMeshReply>* writer) {
    ::mediapipe::Status run_status = RunMPPGraph(writer);
